...
В `web/dist/` находится отдельный статический сайт: главная страница с описанием
и тремя шагами, а также `/fix` — заготовка будущего рабочего интерфейса.
Веб-загрузка и обработка FIT пока не подключены; Core и CLI работают как прежде.
...
Откройте <http://127.0.0.1:4173/>. Сервер перенаправит `/fix` на `/fix/`.
Сайт не требует npm, сборки, внешних шрифтов или сторонних скриптов.
Изображение горной тропы создано специально для лендинга с помощью ImageGen;
оно не содержит реальных пользовательских треков.

Проверка страниц, ссылок и локальных ресурсов:
