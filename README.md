...
Исходный файл не перезаписывается. В выходном FIT сохраняются timestamps и датчики;
существующая правдоподобная высота также не меняется. Меняются только разрешённые
координаты и поддержанные зависимые показатели.
...
Максимальная площадь объединённого OSM-покрытия — `1000` км² на задание;
для CLI её можно изменить через `--osm-max-area-km2`, для Web — через
`WARPBUSTER_WEB_OSM_MAXIMUM_AREA_KM2`.
OSM- и DEM-запросы в режиме `auto` раскрывают внешним поставщикам необходимую
географическую область трека, но не отправляют исходные FIT/GPX, время и датчики.
...
В стандартном `process` включён Task 021D: `--approximate-missing-osm` разрешает
выбор одного hard-safe OSM-маршрута для исходного пропуска координат даже при
неоднозначности. Это приблизительная, не подтверждённая фактическая траектория.
DEM — необязательное свидетельство для выбора среди таких вариантов:

```bash
warpbuster process activity.fit race.gpx --dry-run --json
warpbuster process activity.fit race.gpx --osm-mode offline \
  --dem-snapshot-id 'sha256:<64-значный-хеш>' --dry-run --json
warpbuster process activity.fit race.gpx --no-approximate-missing-osm --json
warpbuster process activity.fit race.gpx --dem-mode disabled --json
warpbuster repair activity.fit --osm-graph-id 'sha256:<graph-id>' \
  --approximate-missing-osm --dem-mode disabled --json
```

Для CLI доступны `--dem-cache-dir` и `--dem-timeout-seconds` (по умолчанию 60).
Без них DEM cache находится в `.warpbuster/dem`. `offline` не загружает тайлы;
без пригодного DEM сохраняется 2D-решение. В `process` и Web режим включён по
умолчанию; legacy `repair` и библиотечные вызовы сохраняют прежние defaults.
`process --osm-mode offline` использует только локальный DEM cache; `disabled`
отключает всю OSM/DEM-цепочку. Политика и DEM status показаны в
JSON, console и HTML, а запись FIT по-прежнему требует validation и FIT diff.

Дополнение высоты (021E) по умолчанию включено в `process` и Web вместе с approximate OSM
и DEM; его можно отключить через `--no-complete-missing-altitude`. Оно заполняет высоту исключительно в выбранном
внутреннем OSM-пропуске, если высота в исходных FIT records отсутствовала. При
полностью отсутствующей высотной серии значения получают явное происхождение DEM
EGM96. Если часть исходных высот уже есть, CLI требует отдельного подтверждения
datum для этого файла: `--fit-altitude-datum egm96`. Неизвестный datum, void DEM,
ошибка sampling или FIT writer оставляют высоту неизменной, не отменяя 2D repair.
