...
> course-driven расширение repair scope: GPX теперь уточняет только локальную
> привязку, не маску изменений. Detector proof этого этапа сохраняется; для MEDIUM
> удаления и MEDIUM пути нужны отдельные разрешения.
