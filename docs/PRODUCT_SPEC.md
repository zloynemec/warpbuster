...
## Текущий FIT-only workflow (Task 022)

Ограничение M5 выше описывает исторический v0.1 provider. В текущем pipeline
`process` принимает один FIT без GPX. После независимой детекции и проверки
минимум 51% пригодного исходного GPS по активному времени внутренние пропуски
могут восстанавливаться через OSM/DEM. Начало или конец можно достроить только
при явно указанной пользователем предполагаемой точке; общая точка допустима
для одного кольца. Точки не имеют времени и не считаются исходными GPS-данными.
Сохранённое правдоподобное движение не меняется. Исходный FIT, coverage gate,
OSM/DEM provenance, оставшиеся разрывы и FIT diff представлены во внутренних
CLI JSON/HTML отчётах. Реализация Web без ручных точек относится к
[Task 023](../tasks/023-fit-only-web.md) после приёмки Task 022: одна страница
с обязательным FIT и необязательным GPX в двух равно заметных блоках.
Ввод старта/финиша или общей точки кольца на карте/координатами вынесен в
[Task 024](../tasks/024-web-start-finish-input.md); пока зафиксирован только сценарий.
